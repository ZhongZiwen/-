...
@interface SMSContactViewController ()<UITableViewDataSource, UITableViewDelegate, UISearchBarDelegate>
...
@implementation SMSContactViewController
...
- (void)selectedButtonPress {
    ...
    ContactSelectedListController *selectedController = [[ContactSelectedListController alloc] init];
    selectedController.title = @"已选择联系人";
    selectedController.sourceArray = [_selectedArray mutableCopy];
    selectedController.refreshBlock = ^(Contact *item) {
        if (item.isSelected) {
            [_selectedArray addObject:item];
        }else {
            for (int i = 0; i < _selectedArray.count; i ++) {
                Contact *tempItem = _selectedArray[i];
        ...
        for (int i = 0; i < self.sourceArray.count; i ++) {
            Contact *tempItem = self.sourceArray[i];
            if ([tempItem.id isEqualToNumber:item.id]) {
...
- (void)sendRequestForIndex {
    [[Net_APIManager sharedManager] request_Contact_Menu_List_WithParams:COMMON_PARAMS andBlock:^(id data, NSError *error) {
        if (data) {
...
- (void)sendRequestForSearch {
    [self.view beginLoading];
    [[Net_APIManager sharedManager] request_Common_SearchList_WithParams:_searchParams path:RELATED_CONTACT block:^(id data, NSError *error) {
...
- (UITableViewCell*)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if ([CommonFuntion isMobileNumber:item.mobile]) {
...
- (void)setSelectedCount:(NSInteger)selectedCount {
    ...
    _bottomLabel.text = [NSString stringWithFormat:@"已选择联系人:%d", _selectedCount];
}
...
- (UITableView *)tableView {
    if (!_tableView) {
        ...
        [self.tableView registerClass:[ContactTableViewCell class] forCellReuseIdentifier:kCellIdentifier];
    }
    return _tableView;
}
