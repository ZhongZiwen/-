...
-(void)hideXLable{
    NSInteger xCount = 0;
    if (_xChartLabels) {
        xCount = [_xChartLabels count];
    }
    for (int i=0; i<xCount; i++) {
        PNChartLabel * label = (PNChartLabel *)_xChartLabels[i];
        label.hidden = YES;
    }
}
...
-(void)hideYLable{
    NSInteger xCount = 0;
    if (_yChartLabels) {
        xCount = [_yChartLabels count];
    }
    for (int i=0; i<xCount; i++) {
        PNChartLabel * label = (PNChartLabel *)_yChartLabels[i];
        label.hidden = YES;
    }
}
...
- (void)touchPoint:(NSSet *)touches withEvent:(UIEvent *)event
{
    ...
    for (NSInteger p = _pathPoints.count - 1; p >= 0; p--) {
        ...
        if ([[CommonStaticVar getFromLLCenterView] isEqualToString:@"llcenter"]) {
            if (linePointsArray == nil || [linePointsArray count] == 0) {
                return;
            }
        }else{
        }
