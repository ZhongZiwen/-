...
@property (nonatomic) BOOL isAllZero;
...
-(void)isAllZeroData{
    ...
    for (int i=0; _isAllZero && i<_items.count; i++) {
    ...
    if(!titleText || self.showOnlyValues){
        ...
        if ([[CommonStaticVar getFromLLCenterView] isEqualToString:@"llcenter"]) {
            ...
            if (_isAllZero) {
                ...
                if ([titleValue floatValue] > 0) {
                    str = [titleValue stringByAppendingString:[NSString stringWithFormat:@"\n%@",titleText]];
                }
            }
