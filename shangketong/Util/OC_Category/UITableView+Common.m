...
//            lineLayer.backgroundColor = self.separatorColor.CGColor;
            lineLayer.backgroundColor = [UIColor colorWithWhite:0.3 alpha:1].CGColor;
    ...
    if (hasData) {
        ...
        if (!self.blankPageView) {
            ...
            if (self.tableHeaderView) {
                origin_y = CGRectGetHeight(self.tableHeaderView.bounds);
                height = height - origin_y;
            }
            self.blankPageView = [[EaseBlankPageView alloc] initWithFrame:CGRectMake(0, origin_y, kScreen_Width, height)];
        }
