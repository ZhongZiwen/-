...
+(void)show:(UIViewController *)handleVC type:(PhotoBroswerVCType)type index:(NSUInteger)index photoModelBlock:(NSArray *(^)())photoModelBlock{
    ...
    ///避免同时点击
    if(appDelegateAccessor.moudle.isShowPhotoView){
        return;
    }else{
        appDelegateAccessor.moudle.isShowPhotoView = TRUE;
    }
...
-(void)show{
    ...
    switch (_type) {
        ...
        case PhotoBroswerVCTypeUserInfo:
...
-(void)viewWillDisappear:(BOOL)animated{
    ...
    if (_type != PhotoBroswerVCTypeUserInfo) {
...
-(void)dismiss{
    ...
    appDelegateAccessor.moudle.isShowPhotoView = FALSE;
