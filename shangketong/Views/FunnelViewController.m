...
@interface FunnelViewController ()<UITableViewDataSource, UITableViewDelegate>
...
- (void)viewDidLoad {
    ...
    NSData *data = [NSData dataWithContentsOfFile:[[NSBundle mainBundle] pathForResource:@"funnel-data" ofType:@"json"]];
...
- (UIView*)customTableHeaderView {
    ...
    // 按钮选项
    FunnelChoiceView *choiceView = [[FunnelChoiceView alloc] initWithFrame:CGRectMake(10, 20 + CGRectGetHeight(chartView.bounds), CGRectGetWidth(headerView.bounds) - 20, 50)];
    choiceView.selectedBlock = ^(NSInteger selectedIndex) {
        NSLog(@"selectedIndex = %ld", (long)selectedIndex);
    };
    [headerView addSubview:choiceView];
    
    [headerView setHeight:CGRectGetHeight(chartView.bounds) + CGRectGetHeight(choiceView.bounds) + 30];
