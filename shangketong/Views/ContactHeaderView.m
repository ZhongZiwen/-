...
@interface ContactHeaderView ()<UIScrollViewDelegate, UICollectionViewDataSource, UICollectionViewDelegate, UICollectionViewDelegateFlowLayout>
...
- (void)configWithObj:(CRMDetail*)item {
    ...
    _nameLabel.text = [NSString stringWithFormat:@"%@ | %@", item.name, item.customer.name];
...
- (void)emailButtonPress {
    ...
    UIAlertAction *sendAction = [UIAlertAction actionWithTitle:[NSString stringWithFormat:@"发送邮件给%@", _detailItem.email] style:UIAlertActionStyleDefault handler:^(UIAlertAction *action) {
...
- (UILabel*)nameLabel {
    if (!_nameLabel) {
        ...
        [_nameLabel setWidth:kScreen_Width - 60];
