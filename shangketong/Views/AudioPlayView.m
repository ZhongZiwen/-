...
- (void)play {
    ...
    if ([_url isFileURL]) {
        ...
        NSString *absUrl = _url.absoluteString;
...
- (void)startDownload:(NSURL *)url {
    ...
    if (![self isDownloading:url]) {
        ...
        NSProgress *progress;
        NSURLSessionDownloadTask *downloadTask = [[FileManager af_manager] downloadTaskWithRequest:request progress:&progress destination:^NSURL *(NSURL *targetPath, NSURLResponse *response) {
            
            NSURL *url =[NSURL fileURLWithPath:[[self class] downloadFile:response.URL.absoluteString]];
            return url;
...
- (BOOL)isDownloading:(NSURL *)url {
    for (NSURLSessionDownloadTask *downloadTask in [FileManager af_manager].downloadTasks) {
