//
//  CustomerHighSeaPoolGroupDetailCell.m
//  shangketong
//
...
@implementation CustomerHighSeaPoolGroupDetailCell
...
-(void)setCellContentDetails:(NSDictionary *)item indexPath:(NSIndexPath *)indexPath{
    ...
    if (releaseReason && ![releaseReason isEqualToString:@""]) {
        self.labelInfo.text = [NSString stringWithFormat:@"%@:%@",infos,releaseReason];
    }else{
        self.labelInfo.text = infos;
    }
}
...
-(void)setCellFrame{
    ...
    self.labelInfo.frame = [CommonFuntion setViewFrameOffset:self.labelInfo.frame byX:0 byY:0 ByWidth:vX byHeight:0];
