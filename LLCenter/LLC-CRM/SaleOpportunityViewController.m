//
//  SaleOpportunityViewController.m
//  lianluozhongxin
//
...
@interface SaleOpportunityViewController ()<UITableViewDataSource,UITableViewDelegate>{
...
@implementation SaleOpportunityViewController
...
-(void)showPopView{
    ...
    if ([[CommonStaticVar getAccountType] isEqualToString:@"boss"]) {
        titles = [NSArray arrayWithObjects:@"新增销售机会",@"删除销售机会", nil];
        imgs = [NSArray arrayWithObjects:@"icon_add_sale.png",@"icon_delete_img.png", nil];
    }else{
        titles = [NSArray arrayWithObjects:@"新增销售机会", nil];
        imgs = [NSArray arrayWithObjects:@"icon_add_sale.png",nil];
    }
...
-(void)gotoDeleteView{
    ...
    if (self.dataSource == nil || [self.dataSource count] < 1) {
        [CommonFuntion showToast:@"暂无销售机会" inView:self.view];
        return;
    }
    
    DeleteSaleOpportunityViewController *controll = [[DeleteSaleOpportunityViewController alloc] init];
    controll.dataSourceOld = self.dataSource;
    
    __weak typeof(self) weak_self = self;
    controll.NotifySaleOpportunitysList = ^{
        listPage = 1;
...
#pragma mark - 读取测试数据
-(void)readTestData{
    id jsondata = [CommonFunc readJsonFile:@"sale_opportunity_data"];
    [self.dataSource addObjectsFromArray:[[jsondata objectForKey:@"body"] objectForKey:@"data"]];
...
-(void)gotoAddOrEditDetailsView:(NSString*)action andIndex:(NSInteger)section{
    ...
    __weak typeof(self) weak_self = self;
    aec.NotifySaleOpportunitysList = ^{
        listPage = 1;
        [weak_self getSaleOpportunityList];
    };
...
- (void)setupRefresh
{
    NSString *dateKey = @"llcsaleopportunity";
...
-(void)getSaleOpportunityList{
    ...
    // 发起请求
    [AFNHttp post:[NSString stringWithFormat:@"%@%@",LLC_SERVER_IP,LLC_GET_SALE_OPPORTUNITY_ACTION] params:rParam success:^(id jsonResponse) {
        [hud hide:YES];
        if(listPage == 1)
        {
        ...
        NSLog(@"销售机会jsonResponse:%@",jsonResponse);
