//
//  DeleteContractViewController.m
//  lianluozhongxin
//
...
#import "DeleteContractViewController.h"
#import "ContractCell.h"
#import "LLCenterUtility.h"
#import "CommonFunc.h"

@interface DeleteContractViewController ()<UITableViewDataSource,UITableViewDelegate>{
...
@implementation DeleteContractViewController
...
-(void)deleteButtonPress {
    NSMutableString *strContractId = [[NSMutableString alloc] init];
    ...
    for (int i=0; i<count; i++) {
        ...
        if ([item objectForKey:@"checked"] && [[item objectForKey:@"checked"] boolValue]) {
            ...
            if ([strContractId isEqualToString:@""]) {
                [strContractId appendString:[item safeObjectForKey:@"contractId"]];
            }else{
                [strContractId appendString:@","];
                [strContractId appendString:[item safeObjectForKey:@"contractId"]];
            }
    ...
    NSLog(@"strContractId:%@",strContractId);
    if ([strContractId isEqualToString:@""]) {
        [CommonFuntion showToast:@"请选择要删除的合同" inView:self.view];
        return;
    }
    
    strDeleteIds = strContractId;
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ContractCell *cell = [tableView dequeueReusableCellWithIdentifier:@"ContractCellIdentify"];
    if (!cell)
    {
        NSArray *array = [[NSBundle mainBundle] loadNibNamed:@"ContractCell" owner:self options:nil];
        cell = (ContractCell*)[array objectAtIndex:0];
        [cell awakeFromNib];
...
-(void)showDeleteAlert{
    UIAlertView *alertCall = [[UIAlertView alloc] initWithTitle:nil message: @"删除当前已选择的合同?" delegate:self cancelButtonTitle:@"取消" otherButtonTitles:@"确认", nil];
    [alertCall show];
}
...
- (void)alertView:(UIAlertView *)alertView clickedButtonAtIndex:(NSInteger)buttonIndex
{
    //删除
    if (buttonIndex == 1) {
        [self deleteContract:strDeleteIds];
    }
}
...
-(void)deleteContract:(NSString *)contractId{
    ...
    [rDict setValue:contractId forKey:@"contractId"];
    ...
    // 发起请求
    [AFNHttp post:[NSString stringWithFormat:@"%@%@",LLC_SERVER_IP,LLC_DELETE_CONTRACT_ACTION] params:rParam success:^(id jsonResponse) {
        [hud hide:YES];
        
        NSLog(@"删除合同jsonResponse:%@",jsonResponse);
        if ([[jsonResponse objectForKey:@"status"] intValue] == 1) {
            ...
            comRequest.RequestAgainBlock = ^(){
                [weak_self deleteContract:contractId];
            };
