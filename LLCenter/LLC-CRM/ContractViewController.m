...
-(void)showPopView{
    ...
    if ([[CommonStaticVar getAccountType] isEqualToString:@"boss"]) {
        titles = [NSArray arrayWithObjects:@"新增合同",@"删除合同", nil];
        imgs = [NSArray arrayWithObjects:@"icon_add_contract.png",@"icon_delete_img.png", nil];
    }else{
        titles = [NSArray arrayWithObjects:@"新增合同", nil];
        imgs = [NSArray arrayWithObjects:@"icon_add_contract.png",nil];
    }
...
-(void)gotoDeleteView{
    ...
    if (self.dataSource == nil || [self.dataSource count] < 1) {
        [CommonFuntion showToast:@"暂无合同" inView:self.view];
        return;
    }
    
    DeleteContractViewController *controll = [[DeleteContractViewController alloc] init];
    controll.dataSourceOld = self.dataSource;
    
    __weak typeof(self) weak_self = self;
    controll.NotifyContractList = ^{
        listPage = 1;
        [weak_self getContract];
    };
...
#pragma mark - 读取测试数据
-(void)readTestData{
    id jsondata = [CommonFunc readJsonFile:@"contract_data"];
...
-(void)gotoDetailsView:(NSInteger)section{
    ...
    sod.contractId = [[self.dataSource objectAtIndex:section] safeObjectForKey:@"contractId"];
...
-(void)getContract{
    ...
    // 发起请求
    [AFNHttp post:[NSString stringWithFormat:@"%@%@",LLC_SERVER_IP,LLC_GET_CONTRACT_ACTION] params:rParam success:^(id jsonResponse) {
        [hud hide:YES];
        if(listPage == 1)
        {
        ...
        NSLog(@"合同jsonResponse:%@",jsonResponse);
