...
@interface DeleteOrderViewController ()<UITableViewDataSource,UITableViewDelegate>{
...
@implementation DeleteOrderViewController
...
-(void)deleteButtonPress {
    NSMutableString *strOrderId = [[NSMutableString alloc] init];
    ...
    for (int i=0; i<count; i++) {
        ...
        if ([item objectForKey:@"checked"] && [[item objectForKey:@"checked"] boolValue]) {
            ...
            if ([strOrderId isEqualToString:@""]) {
                [strOrderId appendString:[item safeObjectForKey:@"orderId"]];
            }else{
                [strOrderId appendString:@","];
                [strOrderId appendString:[item safeObjectForKey:@"orderId"]];
            }
    ...
    NSLog(@"strOrderId:%@",strOrderId);
    if ([strOrderId isEqualToString:@""]) {
        [CommonFuntion showToast:@"请选择要删除的订单" inView:self.view];
        return;
    }
    strDeleteIds = strOrderId;
...
-(void)showDeleteAlert{
    UIAlertView *alertCall = [[UIAlertView alloc] initWithTitle:nil message: @"删除当前已选择的订单?" delegate:self cancelButtonTitle:@"取消" otherButtonTitles:@"确认", nil];
    [alertCall show];
}
...
-(void)deleteOrder:(NSString *)orderId{
    ...
    [rDict setValue:orderId forKey:@"orderId"];
    ...
    // 发起请求
    [AFNHttp post:[NSString stringWithFormat:@"%@%@",LLC_SERVER_IP,LLC_DELETE_ORDER_ACTION] params:rParam success:^(id jsonResponse) {
        [hud hide:YES];
        
        NSLog(@"删除订单jsonResponse:%@",jsonResponse);
